...
class Network(nn.Module):
    def __init__(self,MLP_layers=[32, 32],dic_agent_conf=None, dic_traffic_env_conf=None,len_feature=20):
        ...
        self.fc = HypLinear(self.Poincare,128,self.num_actions,self.c,0,1)
    ...
    def forward(self,feats,adj):
        ...
        # out  = self.MLP(feats)
        out = self.HNN(feats)
        #out = self.Poincare.proj_tan0(self.Poincare.logmap0(out,c=self.c),self.c)
        out = self.fc(out)
        return out
...
class HNN_MLP(nn.Module):
    ...
    def __init__(self, c, manifold,len_feature):
        ...
        self.H_layer1 = HNNLayer(self.manifold,len_feature,128,self.c,0,'relu',1)
        self.H_layer2 = HNNLayer(self.manifold,128,128,self.c,0,'relu',1)
        self.H_layer3 = HNNLayer(self.manifold,128,128,self.c,0,'relu',1)
    def forward(self,x):
