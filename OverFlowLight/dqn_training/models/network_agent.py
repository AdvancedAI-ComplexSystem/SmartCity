...
class Selector:
    def __init__(self, select, d_phase_encoding, d_action):
        self.select = select
...
class NetworkAgent(Agent):
    def __init__(self, dic_agent_conf, dic_traffic_env_conf, dic_path, cnt_round, intersection_id="0"):
        super(NetworkAgent, self).__init__(dic_agent_conf, dic_traffic_env_conf, dic_path, intersection_id)
        
        if cnt_round == 0:
            if os.path.exists(self.dic_path["PATH_TO_MODEL"]) and os.listdir(self.dic_path["PATH_TO_MODEL"]):
            ...
            try:
                ...
                print('Error loading network:', str(e))
        ...
        for i in range(len(sample_slice)):
            ...
            for feature_name in self.dic_traffic_env_conf["LIST_STATE_FEATURE"]:
                if "cur_phase" in feature_name:
                    # 对于相位特征，使用one-hot编码
                    phase = state[feature_name][0]
                    phase_one_hot = np.zeros(8)
                    phase_one_hot[phase-1] = 1
                    dic_state_feature_arrays[feature_name].append(phase_one_hot)
                else:
                    # 对于其他特征，直接使用
            ...
            for feature_name in self.dic_traffic_env_conf["LIST_STATE_FEATURE"]:
                if "cur_phase" in feature_name:
                    ...
                    phase_one_hot = np.zeros(8)
                    phase_one_hot[phase-1] = 1
                    _state.append(np.array([phase_one_hot]))
                    
                    next_phase = next_state[feature_name][0]
                    next_phase_one_hot = np.zeros(8)
                    next_phase_one_hot[next_phase-1] = 1
                    _next_state.append(np.array([next_phase_one_hot]))
                else:
            ...
            if self.dic_agent_conf["LOSS_FUNCTION"] == "mean_squared_error":
                ...
                final_target[action] = reward / self.dic_agent_conf["NORMAL_FACTOR"] + \
    ...
    def convert_state_to_input(self, s):
        ...
        for feature in self.dic_traffic_env_conf["LIST_STATE_FEATURE"]:
            if "cur_phase" in feature:
                ...
                phase_one_hot = np.zeros(8)
                phase_one_hot[phase-1] = 1
                inputs.append(np.array([phase_one_hot]))
            else:
                # 对于其他特征，直接使用
