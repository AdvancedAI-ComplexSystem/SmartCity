...
class Diffusion(nn.Module):
    def __init__(self, state_dim, action_dim, model,
                 ...
                 ddim_timesteps=10, ddim_eta=0.0):
        ...
        self.ddim_timestep_seq = np.asarray(list(range(0, self.n_timesteps, self.n_timesteps // self.ddim_timesteps)))
        self.ddim_timestep_seq = np.append(self.ddim_timestep_seq, self.n_timesteps - 1)
    ...
    # DDIM sampling loop
    def ddim_sample_loop(self, noised_next_state, action, state, shape, return_diffusion=False):
        device = self.betas.device
        batch_size = shape[0]
        x = noised_next_state
        for i in reversed(self.ddim_timestep_seq):
            timesteps = torch.full((batch_size,), i, device=device, dtype=torch.long)
            x = self.p_sample(x, timesteps, action, state)
        return x
    ...
    def predict(self, noised_next_state, action, state, tstp, *args, **kwargs):
        ...
        if self.ddim_timesteps is not None:
            pred_next_state = self.ddim_sample_loop(noised_next_state, action, state, shape, *args, **kwargs)
        else:
            pred_next_state = self.p_sample_loop(noised_next_state, action, state, tstp, shape, *args, **kwargs)
