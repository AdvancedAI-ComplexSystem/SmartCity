...
class RecoveryDataset(Dataset):
    ...
    def __getitem__(self, idx):
        image = self.images[idx]
            ...
            # 生成与 image 形状相同的随机数张量
        random_mask = torch.rand(*image.shape)

        # 将随机数转换为0或1，例如，以0.5为阈值
        mask = (random_mask > 0.5).float()
...
class HINT():
    ...
    def train(self):
        ...
        import pickle
        # 读取.pkl文件
        with open('data_jn_1.pkl', 'rb') as f:  # 注意模式是'rb'（二进制读取）
            data = pickle.load(f)
        from sklearn.model_selection import train_test_split
        states = np.expand_dims(np.array(data[0]['lane_queue_in_part']), axis=1)
        state_s = np.expand_dims(np.array(data[0]['lane_run_in_part']), axis=1)
        images =  np.repeat(np.expand_dims(np.concatenate((states, state_s), axis=1), axis=1), 1, axis=1)
        
   
        
        # 数据集划分
        self.train_dataset = RecoveryDataset(images)
        ...
        while(keep_training):
            ...
            for items in train_loader:
                ...
                if model == 2:
                    ...
                    padded_arr = torch.zeros((self.config.BATCH_SIZE,1,32,32), dtype=images.dtype)
                    padded_arr[:, :, :2, :12] = images
                    images = padded_arr.to(self.config.DEVICE)
                    
                    padded_arr = torch.zeros((self.config.BATCH_SIZE,1,32,32), dtype=images.dtype)
                    padded_arr[:, :, :2, :12] = masks
                    masks = padded_arr.to(self.config.DEVICE)
